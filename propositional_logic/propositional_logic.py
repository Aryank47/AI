...
class FormulaEvaluator:
    ...
    def parse_formula(formula: str, assignment: Dict[str, bool]) -> bool:
        ...
        try:
            # Adding additional safeguards to restrict built-in functions in eval
            allowed_globals = {"__builtins__": None}
            return eval(formula, allowed_globals, assignment.copy())
...
class Resolution:
    ...
    def resolve(clause1: Clause, clause2: Clause) -> Optional[Clause]:
        # Extract literals as sets for efficient operations
        literals1 = set(clause1.literals)
        literals2 = set(clause2.literals)
        complementary_literals = set()

        # Find complementary pairs of literals (one positive and one negative)
        ...
        # If no complementary pairs found, return None
        if not complementary_literals:
            return None

        # Combine remaining literals after removing complementary pairs
        ...
        # Return new clause or None if empty
        # Ensure the result is sorted and has no redundant literals
        return Clause(sorted(result_literals, key=lambda x: x.variable))
...
class InferenceEngine:
    ...
    def is_entailed(self, query: Clause) -> bool:
        ...
        if not isinstance(query, Clause):
            raise ValueError("Query should be a Clause object.")
        ...
        # Iteratively resolve clauses until no new resolvents are found
        for _ in range(self.max_iterations):
            ...
            # If no new clauses were generated, stop the process
...
class TestPropositionalLogic(unittest.TestCase):
    ...
    def test_partial_complementary_pairs(self):
        ...
        expected_clause = Clause([Literal("A"), Literal("B")])
        self.assertEqual(
            str(Resolution.resolve(clause1, clause2)), str(expected_clause)
        )
